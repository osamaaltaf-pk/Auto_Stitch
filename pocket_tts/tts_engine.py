...
class TTSEngine:
    ...
    def generate(
        ...
        stream: bool = False,
        max_tokens: int = 2000,  # Larger default to avoid truncation during full generation
        ...
        """
        # Pre-process text to prevent aggressive clipping at start/end
        ...
        if len(text_clean) > config.MAX_TEXT_LENGTH:
            raise ValueError(f"Text too long. Maximum {config.MAX_TEXT_LENGTH} characters.")
        
        voice_state = self.get_voice_state(voice)
        
        if stream:
            return self._generate_streaming(voice_state, text_clean, max_tokens)
        else:
            return self._generate_complete(voice_state, text_clean)
    
    def _generate_complete(self, voice_state, text: str) -> np.ndarray:
        audio = self.model.generate_audio(voice_state, text, copy_state=True).numpy()
        
        # Peak normalization to prevent clipping distortion ("breaking of words" / "tik tik" noise)
        max_val = np.max(np.abs(audio))
        if max_val > 0:
            audio = (audio / max_val) * 0.95
        
        # Add 0.15s of silence at the start and 0.25s of silence at the end to prevent aggressive clipping
        sample_rate = self.sample_rate
        start_silence_len = int(0.15 * sample_rate)
        end_silence_len = int(0.25 * sample_rate)
        
        start_silence = np.zeros(start_silence_len, dtype=audio.dtype)
        end_silence = np.zeros(end_silence_len, dtype=audio.dtype)
