...
async def render_all(
    ...
    for i in range(n):
        ...
        lip_sync_profile_id = getattr(v_block, 'lip_sync_character_profile_id', None)
        
        if lip_sync_active and lip_sync_profile_id:
            logger.info(f"Slot {i} has lip sync enabled. Generating dynamic lipsync...")
            profile = next((p for p in getattr(manifest, 'character_profiles', []) if p.id == lip_sync_profile_id), None)
            if profile:
                script_val = ""
                if vo_block and getattr(vo_block, 'prompts', None):
                    script_parts = []
                    for p_item in vo_block.prompts:
                        cname = p_item.get("char_name", "Character 1")
                        txt = p_item.get("prompt", "").strip()
                        if txt:
                            script_parts.append(f"[{cname}] {txt}")
                    script_val = " ".join(script_parts)
                elif vo_block and getattr(vo_block, 'prompt', None):
                    script_val = f"[Character 1] {vo_block.prompt}"
                
                if script_val:
                    import httpx
                    lip_sync_url = "http://127.0.0.1:8001/api/generate/lip-sync"
                    payload = {
                        "script": script_val,
                        "image_path": profile.image_path,
                        "chars": [c.to_dict() for c in profile.chars],
                        "char_names": [f"Character {idx+1}" for idx in range(len(profile.chars))],
                        "safe_mode": False
                    }
                    if not is_image:
                        payload["video_path"] = str(v_path)
                        # Extract the landmarks calibration from the primary character config
                        landmarks_calib = None
                        if profile.chars and len(profile.chars) > 0:
                            landmarks_calib = getattr(profile.chars[0], 'landmarks_calib', None)
                        payload["landmarks_calib"] = landmarks_calib
                        
                    try:
                        logger.info(f"Triggering lip-sync request on port 8001: {script_val}")
                        async with httpx.AsyncClient(trust_env=False) as client:
                            resp = await client.post(lip_sync_url, json=payload, timeout=120.0)
                            if resp.status_code == 200:
                                res_data = resp.json()
                                project_root = Path(__file__).resolve().parent.parent.parent
                                source_mp4 = project_root / "lip_sync_standalone" / "static" / "output" / "output.mp4"
                                if source_mp4.exists():
                                    dest_mp4 = output_dir / f"lip_sync_temp_{i:02d}.mp4"
                                    import shutil
                                    shutil.copy2(source_mp4, dest_mp4)
                                    logger.info(f"Generated lip-sync video ready at {dest_mp4}")
                                    v_path = dest_mp4
                                    is_image = False
                                    video_has_audio = True
                                else:
                                    logger.error(f"Lip-sync output file missing: {source_mp4}")
                            else:
                                logger.error(f"Lip-sync API returned {resp.status_code}: {resp.text}")
                    except Exception as ex:
                        logger.error(f"Failed calling lip sync API: {ex}")
