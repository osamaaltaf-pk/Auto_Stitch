...
async def list_available_voices():
    """Fetch the dynamic list of voices from the TTS server, supporting flat list/dict styles."""
    ...
    try:
        ...
        logger.warning(f"Failed contacting TTS voices API: {e}")
    
    # Scanning local text_to_speech_server/uploaded_voices folder as fallback
    try:
        uploaded_dir = BASE_DIR / "text_to_speech_server" / "uploaded_voices"
        local_custom_voices = []
...
async def proxy_delete_voice(voice_name: str):
    """Delete a dynamic cloned voice from the TTS server."""
    settings = load_settings()
    tts_url = settings["tts_server_url"]
    try:
        async with httpx.AsyncClient(trust_env=False) as client:
...
async def proxy_clone_voice(file: UploadFile = File(...)):
    """Upload custom voice sample to TTS and register it as a dynamic cloned voice."""
    settings = load_settings()
    tts_url = settings["tts_server_url"]
    try:
        async with httpx.AsyncClient(trust_env=False) as client:
            ...
            logger.info(f"Proxying voice upload for cloning to TTS at: {tts_url}/api/upload-voice")
