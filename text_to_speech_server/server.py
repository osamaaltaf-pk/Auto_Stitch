...
async def generate_audio(request: GenerateRequest):
    ...
    try:
        # Generate audio inside thread pool to prevent blocking uvicorn event loop
        start_time = time.time()
        loop = asyncio.get_event_loop()
        audio = await loop.run_in_executor(
            None,
            lambda: tts_engine.generate(text, voice, stream=False, max_tokens=max_tokens)
        )
...
async def delete_voice(voice_name: str):
    """Deletes a custom cloned voice file and its cached embeddings."""
    if not voice_name:
    ...
    try:
        ...
        for ext in [".wav", ".mp3", ".flac", ".ogg", ".safetensors"]:
            file_path = config.UPLOADED_VOICES_DIR / f"{voice_name}{ext}"
            if file_path.exists():
                os.remove(file_path)
                deleted = True
                
        # Clear engine cache to free locked file handles & memory
        if tts_engine:
            tts_engine.clear_cache()
            
        if deleted:
            return {"status": "ok", "message": f"Voice '{voice_name}' deleted successfully."}
        else:
